...
function getNextBook(bookId) {
    ...
    bookIdx = bookIdx !== gBooks.length - 1 ? bookIdx + 1 : bookIdx
...
function getPrevBook(bookId) {
    let bookIdx = gBooks.findIndex(book => book.id === bookId)
    bookIdx = bookIdx !== 0 ? bookIdx - 1 : bookIdx
...
function deleteReview(book, review) {
    gBooks = gBooks.map(bookToFind => {
        if (book.id === bookToFind.id) {
            bookToFind = {
                ...bookToFind, rating: bookToFind.rating.filter(currRev => {
                    return currRev.readerName !== review.readerName || currRev.text !== review.text
                })
            }
            return bookToFind
        } else {
...
function addGoogleBook(book) {
    const { title, subtitle, publishedDate, description, pageCount, imageLinks } = book.volumeInfo
    const newBook = new Book(book.id, title,
        subtitle, publishedDate,
        description, pageCount,
        imageLinks.thumbnail, '23', 'USD', true)
